// Test harness for the Claude hooks logger (node:test).
// Each test gets a temp workspace dir; payloads carry cwd = dir
// so all artifacts land under <dir>/.claude/run-logs/<session>/
...
export function useTempWorkspace(t) {
  const dir = mkdtempSync(join(tmpdir(), "claude-logger-test-"));
...
export function writeTranscript(dir, name, envelopes) {
  const p = join(dir, name);
  writeFileSync(p, envelopes.map((e) => JSON.stringify(e)).join("\n"));
  return p;
}

export function sessionLogDir(dir, sessionId) {
  return join(dir, ".claude", "run-logs", sessionId);
}
