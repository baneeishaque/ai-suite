// File IO for the Cline hooks logger. All paths resolve under
// <workspace>/.cline/run-logs/<taskId>/ (workspace = workspaceRoots[0]
// when provided, else process.cwd()). Atomic appends for JSONL, atomic
...
export const RUN_LOGS_DIR = join(".cline", "run-logs");
...
export function serializeLiveTurn(turn) {
  if (!turn) return null;
  return {
