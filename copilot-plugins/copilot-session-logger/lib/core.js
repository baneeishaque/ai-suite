// Copilot logger core — fork of cline-session-logger's core (itself an
// Option A port of opencode-logger.ts Turn/Step model) for GitHub Copilot
// VS Code agent hooks. Sparse mode: hook payloads carry no assistant text;
// tool calls are the primary hook-driven content and the transcript hybrid
// pass (lib/copilot-transcript.js) backfills response text + usage.
// Runtime: node (mise). No bun APIs. No @vscode/* imports.
...
// Copilot hook payloads carry no model field (the requested modelId is
// "copilot/auto" and the actual model surfaces only in the transcript).
// Model identity is harvested by lib/copilot-transcript.js instead.
export function modelFromTranscript(name) {
  if (typeof name !== "string" || name === "") return null;
  return { id: name, provider: "copilot" };
}
...
export class Turn {
  constructor() {
    ...
    this.result = null; // terminal status text (from transcript result details)
    this.error = null; // errorDetails code when the turn failed (e.g. quota_exceeded)
    this.usage = null; // { promptTokens, completionTokens, copilotCredits }
    this.timings = null; // { elapsedMs, timeSpentWaitingMs }
    this.variables = null; // [{ kind, uri, snippet }] from variableData (URIs + capped snippets)
    this.codeBlocks = null; // count of code blocks in the response
  }
  ...
  toFields() {
    ...
    if (this.responseId) out.response_id = this.responseId;
    ...
    if (this.codeBlocks != null) out.code_blocks = this.codeBlocks;
    return out;
  }
}
...
export function newSessionState() {
  return {
    ...
    model: null,
    title: null,
    agent: null, // agent harness id (e.g. github.copilot.editsAgent)
    mode: null, // agent mode ({ kind, permissionLevel, ... })
    ...
    usage: null, // rollup { promptTokens, completionTokens, copilotCredits, outputTokens, nanoAiu, premiumRequests }
    ...
    gitBranch: null, // branch from the live transcript session context
    repo: null, // repository (owner/name) from the live transcript session context
  };
}
...
export function reviveTurn(plain) {
  ...
  t.requestId = plain.requestId ?? null;
  t.responseId = plain.responseId ?? null;
  ...
  t.codeBlocks = plain.codeBlocks ?? null;
...
export function buildDocs(state, taskId) {
  const header = {
    session: {
      ...
      ...(state.repo ? { repository: state.repo } : {}),
