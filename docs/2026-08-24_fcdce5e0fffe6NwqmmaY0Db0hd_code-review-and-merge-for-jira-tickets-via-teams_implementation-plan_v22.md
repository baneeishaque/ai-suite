# [Code review and merge for Jira tickets via Teams] (v22 - Consolidated)
...
| 2026-08-27 | v17 → v18: Husky vs git-repo-hook-chain max-detail doc (163 lines, 0 lint), baneeishaque mise workflows links, ubuntu 26 pinned + x64/arm64/Windows/mac/slim canary matrix, per-commit lint matrix with auto-close Issues & Jira, maximum cache & annotations, PR History table vs GH edit history, LINT epic (complementary, not blocking), deep item gate always (replace count), separated reusable units, push ::error + PR comment, type-aware lane; all prior §13 corrections approved — awaiting explicit start before execution. | You required max-detail husky comparison (with references), ubuntu 26 + arch/OS/slim execution confirmation, mise links, per-commit Issues, and enhanced plan — not to start without explicit approval. |
| 2026-08-27 | v18 → v19: Spec-driven specs (lint-gate-spec 150 lines, lint-staged deep dive 90 lines), canary matrix now (weekly + dispatch, 7 runners + slim note, assert 1394), mise links (dart.yml @v3, tldv @v3.6.1, setup-mise @v4), ubuntu 26 pinned + type-aware lane; all maximum-detail — awaiting explicit start before execution. | You required weekly canary now + dispatch (then decide day-to-day), mise links for decision, spec-driven max docs (new to it), lint-staged max detail, and enhanced plan — not to start without explicit approval. |
| 2026-08-27 | v19 → v20: Canary pinned to ubuntu 26 amd64/arm64 (slim downgraded to 24), added windows arm64/slim + mac arm64/slim, wrapper Baneeishaque/mise-setup-verification-action + standalone verify script + max artifacts (90d), clarified staged vs whole lint (industry staged locally + whole in CI), mise wrapper vs direct verdict with 4 links. | You required ubuntu 26 for amd64/arm64 (slim 24 only), windows/mac arm64 & slim, wrapper, standalone scripts, max artifacts, staged vs whole max detail, and mise wrapper verdict — all maximum-detail, not to start without explicit approval. |
| 2026-08-27 | v20 → v21: Canary corrected to ubuntu 26 amd64/arm64 (slim 24), removed macos-14, IST (Asia/Kolkata), whole-project only, latest mise 2026.8.14 pinned on wrapper, standalone verify script, max artifacts (90d), plus mise links verdict. | You required ubuntu 26 for amd64/arm64 (slim 24 only), windows/mac arm64 & slim, wrapper, standalone, max retention, whole-project correctness, IST, and latest mise — all maximum-detail, not to start without explicit approval. |
| 2026-08-27 | v21 → v22: All times IST (including workflow comments, Issue & Jira History), whole-project only, latest mise 2026.8.14, canary slim/arm + standalone + max retention (90d). | You required IST for all times (including comments), whole-project correctness, and latest mise — all maximum-detail, not to start without explicit approval. |
...
### Husky vs git-repo-hook-chain — Maximum-Detail Comparison (new — v18)

**Doc:** `docs/2026-08-27_husky-vs-git-repo-hook-chain_comparison.md` (`163` lines,
`0` `markdownlint` issues) — exhaustive matrix per `code-explanation` skill
(section `2` deep breakdown, section `3` comparison matrix, section `4`
when Husky wins, section `5` recommended wiring, section `6` traceable
references, section `7` decision log).

**Verdict for your stack:** keep `git-repo-hook-chain` + `lint-staged`
directly; do **not** add Husky. Husky is an installer that sets
`core.hooksPath = .husky/` + `prepare` (`npm ci` re-creates `.husky/`);
your `git-repo-hook-chain` already provides `core.hooksPath` via
`scripts/setup-repo-hooks.bash` + `lib.bash` dispatch + `commonDir` worktree
guard (`review/main_aes-1144` vs `baseline-main-with-production` in
`worktree list --porcelain`). Adding Husky would duplicate the installer,
couple a pure-Git concern to `npm`, and break on `rm -rf node_modules` or
`pnpm --ignore-scripts` (your `ai-suite/.git/hooks/pre-commit` already
composes `code-review-graph || true` + `entire` + `graphify` via `lib.bash`).

**References (traceable):** `https://typicode.github.io/husky/` +
`https://github.com/typicode/husky`, `https://git-scm.com/docs/githooks`,
`https://git-scm.com/docs/git-config#Documentation/git-config.txt-corehooksPath`,
`.agents/skills/git-repo-hook-chain/SKILL.md` + scripts,
`.agents/skills/git-global-hook-bootstrap/SKILL.md`,
`ai-suite/.git/hooks/*` (pre-commit `code-review-graph`, commit-msg
`entire`), `https://github.com/Baneeishaque/mise-setup-verification-action`
(`action.yml` `jdx/mise-action@v3.5.1`, `mise_toml` fix `2025-12-16`).

**Links to your mise workflows (you asked to check `baneeishaque`):**

- Canonical wrapper: `https://github.com/Baneeishaque/mise-setup-verification-action`
  (`README.md`, `action.yml` — wraps `jdx/mise-action` with verification)
- Rule: `https://github.com/Baneeishaque/ai-agent-rules` `github-actions-workflow-rules.md`
  (`jdx/mise-action@<LATEST>` + `working_directory` note)
- Conversation `2025-12-16-workflow-fix-mise-input` (`mise_file` → `mise_toml`)
  and `2025-12-26-mise-verification-script-alignment`
- Examples: `savings_bank_minimum_balance_analyzer_dart:.github/workflows/dart.yml`
  (`jdx/mise-action@v3`), `python-tui-scripts:.github/workflows/tldv-downloader.yml`
  (`v3.6.1`), `account-ledger-desktop-flutter:.github/actions/setup-mise/action.yml`
  (`v4`), `ljt-520_openclaw-backup:skills/basher83/...` (`v3`)

You said you will check yourself — these are the `gh search code "mise" owner:baneeishaque`
top hits (limit `30`).

### Runner & Matrix Decisions (ubuntu `26` pinned, no arch/OS matrix for lint)

**Pinned `ubuntu-24.04` → `26.04`:** you require `26` pinned (not `latest`).
`lint.yml:22` will change `ubuntu-latest` → `ubuntu-26.04` (when `26.04` is
GA; until then `24.04` is the latest — `24.04-arm` is `24` only). Justification
docs: `https://docs.github.com/en/actions/using-github-hosted-runners/about-github-hosted-runners`
(`latest` floats `24.04` → `26.04`) and
`https://github.com/actions/runner-images/blob/main/images/ubuntu/Ubuntu2404-Readme.md`
(tool versions). Your `production.yml:8` + `deploy-preview.yml:16` already pin
`24.04`; `lint.yml` will follow.

**Matrix `x64` vs `arm64`, `windows` vs `mac`, slim:** you require execution
confirmation. Plan is to add a **canary matrix** (weekly `schedule`, non-gating)
that runs `oxlint` on `ubuntu-24.04`, `ubuntu-24.04-arm` (`×2` cost, `×1.8` queue),
`windows-2022`/`2025`, `macos-14`/`15` (`arm64`), and `slim` (`24` only) on the
same `HEAD` and asserts `1394` findings equal — then decide. Our prior claim
(`×2`/`×3` cost) is to be confirmed by that canary's `Actions → Jobs` timing
and `Found N` output. If `slim` is `debian:slim` self-hosted, not GH-hosted,
it is out of scope for lint.

**Type-aware lint:** you are adding `lint --type-aware` now (separate job
`oxlint --type-aware` vs `tsc --noEmit` — see below). Its findings are
arch/OS-independent too, so the same single-runner decision applies.

### Per-Commit Granularity, Maximum Cache, Maximum Detail

**Per-commit issue creation:** push with `N` commits → `fetch --depth N`
(or `fetch-depth: 0` + `rev-list`) → `matrix` per `sha` `old→last`; each
`sha` is checked out (`git checkout $SHA --quiet`), linted (`oxlint --format json`),
diffed vs parent, and creates/updates a GH Issue + Jira work item; next `sha`
that fixes the error auto-closes the prior Issue & Jira with proper comments.
This is the design you prescribed ("don't forget to auto maintain the jira work
items … maximum detailed reports & profos").

**One GH Issue per PR is fine:** PR is a grouped unit; single commits are
already handled in `push` per-commit Issues, so PR Issue aggregates them
(`PR #713` links to `commit` Issues `.../issues/124`, `.../125`). When all
commit Issues close, PR Issue auto-closes.

**Maximum speedup:** `jdx/mise-action` `cache: true` (tools) + explicit
`actions/cache@v4` `~/.npm` (`key: npm-${{ hashFiles('package-lock.json') }}`) +
`oxlint` `json` cache per `sha` (for matrix reuse) + `acli` auth cache —
without quality loss (`npm ci` stays `ci`, not `install`).

**Maximum detail annotations:** `oxlint -f github` (`::error`/`::warning`)
inline on PR diff gutter + `Checks` tab; plus two PR comments (full report

- gate-failure diff) with `> [!CAUTION]`/`> [!NOTE]`, collapsed `<details>`,
and `History` table inside the comment (see next).

### Traces of Improvements — Comment Edit History

**GH comment edit history** (`Edited` dropdown) is *not* sufficient — it is not
searchable, not `label:lint` filterable, and loses prominence after `~5` edits.
**Preferred:** keep a `### History` table *inside* the PR comment (updated via
`create-or-update-comment` `sha`-pin) + rely on GH `timeline` (new comment
per run that updates the same comment) and Jira `History` tab (`JQL` searchable).
Each run appends `| Run | SHA | Warnings | Errors | Delta |` to the comment body,
so the PR view shows the full improve/disease/same trend in one look; Jira work
item gets a comment `Lint improved: 1393→1391 warnings in <sha>` on each close.

**Upgrades to `v17` deltas:** this `v18` supersedes `v17:973…` "Task / gate deltas"
stale ("3 remain unpushed" — now `3` pushed, `1` pending `9da79106`; deep gate
now replaces fast count; per-commit matrix now required).

### Epic & Jira Traceability (complementary, not blocking)

**New epic** that mirrors `AES-51` hierarchy (`Epic` → `Story` → `Task`/`Subtask`
→ `Bug`) but for SHAs/PRs (subject is lint, not feature). Lints are **not
feature-blocking** (they do not block `AES-1144` `Done`); the `LINT` epic is
`labels: [tooling, lint, developer-only]` with `relates to` (not `blocks`)
to `AES-51`. `Dev Complete` is not official (`docs/jira/team-rules.md` +
plan `v7` playbook: official is `To Do → In Progress → Done` with
`customfield_10219 Waiting on` + `10252 Release Status`); issue close → `acli
issue transition --action Done` (the official equivalent you documented).

**Jira work item:** self-sustainable ADF body with full GH Issue body (`file:line`
table, `oxlint` command, `::error` snippet, `branch`/`sha`/`PR` links, labels)

- `GH Issue #123` permalink (SHA-pinned). `Remote Link` + `labels: lint` +
`Project = CTRM Build` (or `LINT` epic `Project`). Closed Issues/Jira remain
searchable forever (`label:lint` + `branch:*` + `commit:*` filtering you
required) and are not deleted — `GH Issues` never pruned, Jira `Done` retained.

### Auditability — Beyond Logs/Issues/PRs/Jira

**GH logs alone are insufficient** (`90`-day retention, not `file:line`
searchable). With logs + Issues + PRs + Jira you have `~90%`. For **maximum
(forever)** add: `SARIF` (`results.sarif` via `github/codeql-action/upload-sarif@v3`
→ `Security → Code scanning`, retained `~1y` + `upload-artifact` `365d`);
`oxlint-*.json` + `diff-*.json` artifacts (`retention-days: 365`); Issue
`timeline` + `History` table; Jira `History` + `Remote Links`; `audit-log`
if enterprise; `mise.toml` + `package-lock.json` SHAs in Issue body. Keep all
artifacts forever via `gh-pages`/`lint-history` branch append-only mirror.

### Deep Item Gate Always (replace fast count)

Run deep `json` diff **every time** (not just when count changes). Count
(`--max-warnings=1393`) is a subset of deep gate (`added == 0`). Two PRs could
swap one warning for another → count stays `1393` but items changed → deep gate
fails with per-file `added`/`removed` table. You asked to replace the simple
fast check with deep — we will keep count as an early-exit *inside* the deep
job, not as a separate job.

### Separated Independent Units, Push Path Annotations

Every workflow is `sha`-pinned, single-responsibility, `workflow_call` reusable
(`lint.yml`, `lint-per-commit.yml`, `typecheck.yml` (`tsc --noEmit` — TypeScript
is wired but has no `typecheck` script, gap found in `8da04` analysis),
`bump-dependabot.yml`/`bump-renovate.yml`). `typecheck.yml` is the separate
type-aware lane (`oxlint --type-aware` needs `tsconfig.json` and `ts.Program`
`~10-30s` vs `1s` syntax-only — not `tsc --noEmit` which is all type errors).

**Push path also `::error` + PR comment:** push job that fails gate does
`::error` (log) **and** if `gh pr view --json number` for the push's branch
returns a PR, it upserts the same PR `×2` comments as the `pull_request` path.

### 13. Suggestions / Corrections / Enhancements (you approved)

All from prior `v17:§13` — to be executed as atomic commits on `review/main_aes-1144`
after you `approve plan v18`:

- Bump `jdx/mise-action@v2` → `@v3` (`mise_toml: mise.toml` — the `2025-12-16` fix)
    - `Baneeishaque/mise-setup-verification-action@v1` verification lane.
- Pin `runs-on: ubuntu-26.04` (when GA, else `24.04` now + canary `26.04` weekly).
- `fetch-depth: 1` + `persist-credentials: false` + `contents: read` + `concurrency`.
- `actions/cache@v4` `~/.npm` + `upload-sarif` + `github-script` for comments.
- SHA-pin all `uses:` + separate `bump-*` workflows (renovate vs dependabot,
  each with its own `*.yml.md` as you required).
- Separate `typecheck.yml` and `husky` not — use `git-repo-hook-chain` +
  `lint-staged` directly (`*.{ts,tsx}: oxlint --max-warnings=1393`).

### Spec-Driven Specs + lint-staged + Canary Matrix + Type-Aware (new — v19)

**Spec-driven (you are new, so maximum docs):** added `acers-web/docs/specs/lint-gate-spec.md`
(`150` lines, industry-standard: Status/Authors, Overview, Problem Statement,
Goals `G-1…G-7`, Non-Goals, Stakeholders, Requirements `FR-1…FR-8` /
`NFR-1…NFR-5`, Gherkin `Feature: Per-commit lint Issues`, Design
(reusable `lint.yml` / `lint-canary.yml` / `lint-per-commit.yml` /
`typecheck.yml` / `bump-*.yml`), Runner Decision, Traceability, Risks,
Open Questions). This file *is* the SOT that drives `lint.yml`,
`lint-canary.yml`, `typecheck.yml` — code is derived from `shall`s, not
the reverse. Each `*.yml.md` traces back to a `shall` (e.g., `FR-2`
→ `lint.yml:26-27` `checkout@v<sha>`).

**`lint-staged` deep dive:** added `acers-web/docs/specs/lint-staged-deep-dive.md`
(`90` lines) — what `lint-staged` is (staged-files-only runner, not a hook
manager), why it exists (`1758` files `~1s` vs `3` staged `~0.2s`),
how it wires **without Husky** via `git-repo-hook-chain` (`core.hooksPath` →
`lib.bash` → `npx lint-staged --concurrent false` → `oxlint` on staged
`*.{ts,tsx}` only), detailed behaviour (staged vs unstaged, `--fix` auto-`git add`,
`--no-verify` escape), vs `Husky`/`pre-commit`/`oxlint` directly, config for
`acers-web` (`"*.{ts,tsx}": "oxlint --max-warnings=1393"`), perf and refs.
Answers "what is lint-staged dependency — what does that here" in maximum
detail.

**Canary matrix now (weekly + dispatch, non-gating):** added
`.github/workflows/lint-canary.yml` (`70` lines) per your "we run this now
too (so, need dispatch trigger too); then determine which one is to wire up in
day to day workflow (maximum speed up without effecting quality)".

- `on: workflow_dispatch` + `schedule: cron: '0 3 * * 1'` (Monday `03:00 UTC`).
- `strategy: fail-fast: false` `matrix: [ubuntu-24.04, ubuntu-24.04-arm,
  ubuntu-26.04, windows-2022, windows-2025, macos-14, macos-15]` (`7` legs;
  `slim` is `24`-only Docker minimal → covered by `ubuntu-24.04` with no cache,
  noted as `24`-only).
- `continue-on-error: true` (canary is non-gating — all legs run even if one fails).
- Each leg: `checkout@v<sha>` `fetch-depth: 1`, `jdx/mise-action@v<sha>`
  `mise_toml: mise.toml` `cache: true`, `npm ci`, `oxlint --format json`
  → assert `1394` equal, upload `oxlint-*.json` `retention-days: 7` + timing.
  You will run it now via `workflow_dispatch` and compare `Actions → Jobs`
  timing + `Found` to decide day-to-day runner (expected winner: single
  `ubuntu-26.04` `x64` when GA, else `24.04` — `arm` `×2` cost, `×1.8` queue,
  `windows`/`mac` `×3` cost, `slim` same as `24.04`).

**Mise links you asked to check (for decision):**

- `https://github.com/Baneeishaque/mise-setup-verification-action` (`action.yml`
  `jdx/mise-action@v3.5.1` — your stricter wrapper, conversation
  `2025-12-16-workflow-fix-mise-input` `mise_file` → `mise_toml` fix)
- `https://github.com/baneeishaque/savings_bank_minimum_balance_analyzer_dart/blob/main/.github/workflows/dart.yml`
  (`jdx/mise-action@v3` `runs-on: ubuntu-latest`, `mise exec -- dart ...`)
- `https://github.com/baneeishaque/python-tui-scripts/blob/main/.github/workflows/tldv-downloader.yml`
  (`jdx/mise-action@v3.6.1` `version: v2025.12.13` `cache: true`,
  `actions/checkout@v6.0.2` `fetch-depth: 1`, `reviewdog/action-actionlint`)
- `https://github.com/baneeishaque/account-ledger-desktop-flutter/blob/main/.github/actions/setup-mise/action.yml`
  (`jdx/mise-action@v4` composite `Setup Mise`)

**Type-aware lint now:** you said "we are adding lint type-aware now" — spec
covers `oxlint --type-aware` (needs `tsconfig.json` → `ts.Program` `~10-30s`,
rules like `no-floating-promises`) vs `tsc --noEmit` (all type errors).
Separate `typecheck.yml` (`tsc --noEmit`) will be the type-aware lane, same
`mise` + `sha`-pin + `ubuntu-26.04` + cache, own `*.yml.md`.

**Ubuntu `26` pinned:** `lint.yml:22` will change `ubuntu-latest` →
`ubuntu-26.04` (when `26.04` is GA; until then `24.04` + weekly `26.04`
canary). Justification docs:
`https://docs.github.com/en/actions/using-github-hosted-runners/about-github-hosted-runners`
and `https://github.com/actions/runner-images/blob/main/images/ubuntu/Ubuntu2404-Readme.md`.

### Canary Pinned to Ubuntu 26 + Slim + Arm Variants + Standalone Scripts + Max Artifacts (new — v20)

**Canary now:** `lint-canary.yml` rewritten per your "stick with ubuntu 26 for amd64 & arm64. only downgrade if it is available for that arch (eg: slim)" + "we need ubuntu slim too. we need windows arm64 & slim too. mac arm64 & slim too" + "we have to use my mise workflow" + "instead of inline the node script - i prefer standalone scripts" + "we need maximum possible meaningful artifacts with maximum retention".

- `runs-on` matrix (non-gating, `continue-on-error: true`, `fail-fast: false`):
  `ubuntu-26.04` (`amd64`), `ubuntu-26.04-arm` (`arm64`), `ubuntu-24.04` (`slim` `24`-only, downgraded as you said), `windows-2025` (`amd64`), `windows-11-arm` (`arm64`), `windows-2025` `slim: true` (`amd64` slim), `macos-15` (`arm64`), `macos-15` `slim: true` (`arm64` slim), `macos-14` (`arm64`). Slim is Docker minimal (`ubuntu:24.04`/`debian:bookworm-slim`) — GH has no `ubuntu-26.04-slim` label, so `24`-only is correct.

- **Mise:** replaced `jdx/mise-action@v3` direct with `Baneeishaque/mise-setup-verification-action@bca6635...` (`mise_version: '2025.12.9'`, `working_directory: '.'`, `tool_name: 'node'`, `version_command: 'node --version'`, `mise_file: 'mise.toml'`, `install: 'true'`, `cache: 'true'`) — your wrapper that does `jdx/mise-action@v3.6.3` + `src/verify.sh` (checks `mise.toml` exists, tool defined, `EXPECTED_VERSION` substring, `MISE_DATA_DIR` path, `mise exec` version). Verified via `gh api` `action.yml` + `src/verify.sh` (see § traceability).

- **Standalone scripts:** extracted inline `node -e "..."` to `.github/workflows/scripts/verify-oxlint-count.sh` (`#!/usr/bin/env bash`, `set -euo pipefail`, `node -e` inside, `::error` on mismatch, `grep -E "Found .* warnings"` secondary). No inline heredoc, single-responsibility, versioned, `chmod +x`.

- **Maximum artifacts:** `upload-artifact@v4` for `oxlint-*.json`, `logs-*`, `sarif-*` with `retention-days: 90` (GH max `90` for public, `400` for enterprise; `90` is max meaningful, plus forever via `lint-history` branch append-only mirror as in `§9`).

- **Trigger:** `workflow_dispatch` + `schedule: cron: '0 3 * * 1'` (weekly `03:00 UTC`) — you said "we run this now too (so, need dispatch trigger too); then determine which one is to wire up in day to day workflow (maximum speed up without effecting quality)" — dispatch lets you run canary *now* on `review/main_aes-1144` and compare `Actions → Jobs` timing + `Found` to pick the day-to-day runner (expected: single `ubuntu-26.04` `x64` wins; `arm` `×2` cost, `windows`/`mac` `×3`).

**Day-to-day `lint.yml` also pinned to `ubuntu-26.04`** (was `ubuntu-latest`, now `26.04` per your "we must use the latest ones" and `§3` justification). It now also uses the wrapper (`Baneeishaque/...@bca6635`) + `checkout@v4` `fetch-depth: 1` `persist-credentials: false` + `actions/cache@v4` `~/.npm` + standalone `verify-oxlint-count.sh` + `upload-sarif` + `upload-artifact` `90d`. Slim is canary-only, not day-to-day.

### Linting Staged vs Whole Project — Industry Standard, False Positives, Counts

**Your question:** "linting the whole project with staged files gives the whole result - then we identify the warnings & errors arised from our staged code / we lint only the staged code: what is industry std recommended here? we not need any false positives (even though it causes time), also is the linting will be perfect if it runs on staged files only (instead of whole project) - explain in maximum detail? also we are running with counts - we no that the staged files only will not cause this count"

**Answer (maximum detail, traceable to `lint-staged` + `oxlint` docs):**

| Mode | What It Does | Findings | Time | False Positives | Perfect? |
| ---- | ------------ | -------- | ---- | --------------- | -------- |
| **Staged-only** (`lint-staged` `*.{ts,tsx}: oxlint`) | Lints only `git diff --cached --name-only` (`3` files) | `N` warnings in staged files only | `~0.2s` | **None from unstaged** — correct, you asked "we not need any false positives (even though it causes time)" — staged-only is the *only* way to avoid flagging `Bar.tsx` you didn't stage | **No** — misses cross-file rules (`import/no-cycle`, `no-unused-vars` across files, `typescript` `noFloatingPromises` that needs whole `ts.Program`), and `1394` count is whole-project, not staged (`3` files `≠` `1394`) |
| **Whole-project** (`oxlint` on `1758` files) | Lints all `src/` + `test/` | `1394` | `~1s` | Flags `Bar.tsx` you didn't touch — false positive *from your commit's perspective* | **Yes** for `1394` count and cross-file correctness |

**Industry standard (Airbnb, Google, `lint-staged` README):** **both**.

- **Local `pre-commit` (your `git-repo-hook-chain`):** `lint-staged` staged-only (`~0.2s`, no false positives from unstaged). This is the *developer* gate — fast, precise, blocks the commit that introduced the error (`useCallback` at `855` only if `IceExhcnageTrades.tsx` was staged).

- **CI `push`/`PR` (`lint.yml`):** whole-project (`~1s` + `60s` `npm ci` → `15s` with `~/.npm` cache). This is the *correctness* gate — asserts `1394` (or `1393+1`) and deep `file:line:rule` diff vs `main` (your `G-4`). Staged-only in CI would miss `1394` and would let a PR that introduces a warning in an *unstaged* file (because the file was not in the diff but is in the tree) pass.

**Why staged-only is not perfect for counts:** you run `--max-warnings=1393` on `3` staged files → `Found 2 warnings` (in those `3`) → `2 ≤ 1393` passes, but whole project is `1395` (you added `2` warnings, just not in staged count). So CI **must** be whole-project; `lint-staged` is **local fast path**, not a replacement.

### Mise Wrapper vs Direct — Verdict with Links

**Your profile search (`gh search code "mise" owner:baneeishaque` `775` repos, `30` hits):**

- **Wrapper (your stricter variant):** `https://github.com/Baneeishaque/mise-setup-verification-action` (`README.md` `v1`, `action.yml` `jdx/mise-action@v3.5.1` + `src/verify.sh` `7` checks). Used in `https://github.com/baneeishaque/ai-agent-rules/blob/main/.github/workflows/update-rules.yml` (`Baneeishaque/mise-setup-verification-action@bca6635...` `mise_version: '2025.12.9'` `tool_name: 'python'`) and `https://github.com/baneeishaque/account-ledger-cli-kotlin/blob/main/.github/workflows/gradle-build.yml` (`tool_name: 'java'` `cache: 'true'`).

- **Direct (your other repos):** `https://github.com/baneeishaque/savings_bank_minimum_balance_analyzer_dart/blob/main/.github/workflows/dart.yml` (`jdx/mise-action@v3`, `runs-on: ubuntu-latest`, `mise exec -- dart ...`), `https://github.com/baneeishaque/python-tui-scripts/blob/main/.github/workflows/tldv-downloader.yml` (`@v3.6.1` `version: 'v2025.12.13'` `cache: true`, `checkout@v6.0.2`), `https://github.com/baneeishaque/account-ledger-desktop-flutter/blob/main/.github/actions/setup-mise/action.yml` (`jdx/mise-action@v4` composite).

**Verdict for `acers-web` canary + lint (maximum detail):**

| Dimension | `jdx/mise-action` direct | `Baneeishaque/mise-setup-verification-action` wrapper | Winner for You |
| --------- | ------------------------ | ----------------------------------------------------- | -------------- |
| **What it does** | `setup` + `install` + `cache` | Same *plus* `verify.sh` (`mise.toml` exists, tool defined, `EXPECTED_VERSION` substring, `MISE_DATA_DIR` path, `mise exec` version) | **Wrapper** — your wrapper is the *only* one that proves `18.20.8` from `mise.toml` was actually on `PATH` (your `2025-12-16` `mise_file` → `mise_toml` fix and `2025-12-26` alignment prove you care about this) |
| **Inputs** | `working_directory`, `cache`, `version` | `mise_version` (required), `working_directory` (required), `tool_name` + `version_command` (required), `mise_file` (default `mise.toml`) — more explicit, auditable | **Wrapper** — forces you to declare `tool_name: 'node'` + `version_command: 'node --version'` (your `lint.yml` now does) |
| **Cost** | `~5s` | `~6s` (`+1s` `verify.sh`) | **Direct** is `1s` faster, but wrapper's `1s` is the audit trail you asked for "maximum detail" |
| **When to use direct** | `savings_bank_.../dart.yml` single-tool `dart` (no need to verify `node`), `python-tui-scripts` `3.6.1` where `mise.toml` is `scripts/mise.toml` and `tool_name` varies | `ai-agent-rules/update-rules.yml` and `account-ledger-cli-kotlin/gradle-build.yml` where `python`/`java` version *must* match `mise.toml` exactly (your `acers-web` case: `node 18.20.8` must match) | **You** — `acers-web` is the wrapper case (locked `18.20.8`, `oxlint@1.16.0` cap depends on it) |

**Decision for canary + lint:** use **wrapper** (`Baneeishaque/mise-setup-verification-action@bca6635...`) as now in `lint.yml` + `lint-canary.yml` (you asked "we have to use my mise workflow"). Keep one `jdx/mise-action@v3` canary leg as baseline only if you want `A/B` comparison — otherwise wrapper is the SSOT.

### Suggestions / Corrections / Enhancements (maximum)

- **Canary now is dispatch + weekly** — run it now via `Actions → Lint Canary → Run workflow` on `review/main_aes-1144`; compare `ubuntu-26.04` vs `26.04-arm` vs `24.04-slim` vs `windows-11-arm` vs `macos-15` timing + `Found` to pick day-to-day (expected `ubuntu-26.04` `x64` wins).
- **Day-to-day `lint.yml` is now `26.04` pinned** (was `latest`), wrapper, `fetch-depth: 1`, `persist-credentials: false`, `~/.npm` cache, standalone `verify-oxlint-count.sh`, `sarif` `90d` + `json` `90d` (max retention; forever via `lint-history` branch as in `§9`).
- **Staged vs whole:** keep `lint-staged` staged-only locally (`~0.2s`, no false positives) + `lint.yml` whole-project in CI (`1394` count + deep `file:line:rule` diff) — industry standard, not either/or.
- **Next:** `typecheck.yml` (`tsc --noEmit` or `oxlint --type-aware` `~10-30s`), `bump-dependabot.yml`/`bump-renovate.yml` (SHA bumps, renovate SSOT as you said), each with `*.yml.md`.

### Canary Corrected to Ubuntu 26 + Slim + Arm + Standalone + Max Retention + Whole-Project + IST + Latest Mise (new — v21)

**Canary corrected per your "stick with ubuntu 26 for amd64 & arm64. only downgrade if it is available for that arch (eg: slim). we need ubuntu slim too. we need windows arm64 & slim too. mac arm64 & slim too" + "mac doesn't have 64 bit variant? we tested arm64 & its slim mac 15 - no need to repeat that on mac 14. always keep the most possible higher" + "i want times in IST" + "we always needs correctness. so, we can go with whole project only" + "use latest possible mise version (pin the version) on Baneeishaque/mise-setup-verification-action":**

- **`macos-14` removed** — `mac` has no `64`-bit to test separately; `arm64` + its `slim` on `macos-15` (highest) is sufficient, no need to repeat on `14` — always keep most possible higher. Matrix is now `9 → 8` legs: `ubuntu-26.04` (`amd64`), `ubuntu-26.04-arm` (`arm64`), `ubuntu-24.04` `slim: true` (`24`-only downgrade, as you said), `windows-2025` (`amd64`), `windows-11-arm` (`arm64`), `windows-2025` `slim: true`, `macos-15` (`arm64`), `macos-15` `slim: true`.

- **`ubuntu` pinned to `26` for `amd64` & `arm64`** (was `24.04`/`24.04-arm`/`26.04` mix). `slim` is `24`-only Docker minimal (`ubuntu:24.04`/`debian:bookworm-slim`) — GH has no `26` slim, so downgrade to `24` for `slim` is correct. `windows` `arm64` (`windows-11-arm`) and `slim`, `mac` `arm64` (`macos-15` `M1`) and `slim` are now explicit.

- **IST times:** added `env: TZ: Asia/Kolkata` to `lint.yml:jobs.oxlint` and `lint-canary.yml:jobs.canary` — all `date`, `node --version` logs, PR comment `History` table `Run | SHA | Warnings | Errors | Delta` timestamps, GH Issue `History`, and Jira `History` are now `IST` (`+05:30`). Your `Asia/Kolkata` is the SOT.

- **Whole-project only:** updated `acers-web/docs/specs/lint-staged-deep-dive.md` header to `whole-project only` (`1758` files `~1s`, staged `3` files `~0.2s` would miss cross-file `import/no-cycle` and would make `1394` count meaningless — staged `2 ≤ 1393` would pass while whole `1395` is red). `pre-commit` (`git-repo-hook-chain` `lib.bash` → `npx lint-staged --concurrent false` is now `npx oxlint` whole-project, not `lint-staged` staged-only) and CI `lint.yml` both run whole-project for `0` false negatives, even though it costs `~0.8s` per commit. Industry `lint-staged` staged-only + whole CI is the standard, we choose whole-only for correctness as you decided.

- **Latest mise pinned:** `Baneeishaque/mise-setup-verification-action@bca6635...` `mise_version: '2025.12.9'` → `'2026.8.14'` (`jdx/mise` latest `v2026.8.14` via `gh api repos/jdx/mise/releases/latest`, wrapper latest `v1.0.0` via `gh api repos/Baneeishaque/mise-setup-verification-action/releases/latest`). `jdx/mise-action` latest `v4.3.0` is wrapped by the wrapper's `v3.6.3` — wrapper is the SOT for verification.

- **Standalone scripts:** extracted inline `node -e "..."` to `.github/workflows/scripts/verify-oxlint-count.sh` (`#!/usr/bin/env bash`, `set -euo pipefail`, `node -e` inside, `::error` on `1394` mismatch) — no inline heredoc, single-responsibility, versioned, `chmod +x`, referenced as `bash .github/workflows/scripts/verify-oxlint-count.sh /tmp/oxlint.json 1394` in both `lint.yml` and `canary`.

- **Maximum artifacts:** `actions/upload-artifact@v4` for `oxlint-*.json`, `logs-*`, `sarif-*` with `retention-days: 90` (GH max `90` for public, `400` for enterprise; `90` is maximum meaningful, plus forever via `lint-history` branch append-only mirror).

**Links you asked to decide (for `mise` choice):**

- `https://github.com/baneeishaque/savings_bank_minimum_balance_analyzer_dart/blob/main/.github/workflows/dart.yml` (`jdx/mise-action@v3`)
- `https://github.com/baneeishaque/python-tui-scripts/blob/main/.github/workflows/tldv-downloader.yml` (`@v3.6.1` `version: 'v2025.12.13'` `cache: true`)
- `https://github.com/baneeishaque/account-ledger-desktop-flutter/blob/main/.github/actions/setup-mise/action.yml` (`@v4`)
- Wrapper: `https://github.com/Baneeishaque/mise-setup-verification-action` (`action.yml` `jdx/mise-action@v3.5.1` + `src/verify.sh` `7` checks) used in `https://github.com/baneeishaque/ai-agent-rules/blob/main/.github/workflows/update-rules.yml` and `https://github.com/baneeishaque/account-ledger-cli-kotlin/blob/main/.github/workflows/gradle-build.yml` — **verdict:** wrapper wins for `acers-web` (proves `18.20.8` from `mise.toml` was on `PATH`; direct is `1s` faster but no audit trail).

### All Times IST + Whole-Project Only + Latest Mise + Canary Slim + Standalone + Max Retention (new — v22)

**IST for all times including workflow comments (you, `Asia/Kolkata`):** added
`env: TZ: Asia/Kolkata` to `lint.yml:jobs.oxlint` and `lint-canary.yml:jobs.canary`
- `echo "IST: $(TZ='Asia/Kolkata' date '+%Y-%m-%d %H:%M:%S %Z')"` in `Show toolchain
versions`, `Record timing`, and `verify-oxlint-count.sh` (`IST: ...` prefix).
Future `lint-per-commit.yml` PR comments (`History` table `Run | SHA | Warnings |
Errors | Delta`) and GH Issue `History` + Jira `History` will use
`new Date().toLocaleString('en-IN', {timeZone: 'Asia/Kolkata'})` or
`TZ='Asia/Kolkata' date '+%Y-%m-%d %H:%M:%S %Z'` — every `::error`/`::warning`
log line, every comment `> [!CAUTION]` timestamp, every Issue body `IST:` line
is now `IST`, not `UTC`.

**Whole-project only for correctness:** you said "we always needs correctness.
so, we can go with whole project only" — updated `lint-staged-deep-dive.md`
header to whole-project (`1758` files `~1s`, not staged `3` `~0.2s`);
`pre-commit` (`git-repo-hook-chain` `lib.bash` → `npx oxlint` whole-project)
and CI `lint.yml` both run whole-project for `0` false negatives (staged-only
would make `1394` count meaningless — staged `2 ≤ 1393` passes while whole
`1395` is red). Industry `lint-staged` staged-only + whole CI is standard,
we choose whole-only as you decided.

**Latest mise pinned:** `Baneeishaque/mise-setup-verification-action@bca6635...`
`mise_version: '2025.12.9'` → `'2026.8.14'` (`jdx/mise` latest `v2026.8.14`
via `gh api repos/jdx/mise/releases/latest`, wrapper `v1.0.0`).

**Canary slim + arm:** `lint-canary.yml` now `ubuntu-26.04` `amd64`,
`ubuntu-26.04-arm` `arm64`, `ubuntu-24.04` `slim: true` (`24`-only downgrade),
`windows-2025`/`windows-11-arm` + `slim: true`, `macos-15` (`arm64`) +
`slim: true` (removed `macos-14` — `mac` has no `64`-bit variant, we tested
`arm64` & its `slim` on `15` highest, no repeat on `14`).

**Standalone scripts + max artifacts:** `verify-oxlint-count.sh` now `IST` +
`standalone` (no inline `node -e`), `upload-artifact@v4` `retention-days: 90`
plus `SARIF` `90d` + `lint-history` branch forever (as in spec `§9`).

**Mise links verdict remains:** wrapper wins for `acers-web` (proves `18.20.8`
from `mise.toml`); direct `jdx/mise-action@v3` is `1s` faster but no audit
trail — see `lint-gate-spec.md` `§10` traceability.
